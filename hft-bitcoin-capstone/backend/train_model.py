...
# plt.show()#BUNUN YERİNE ALTTAKİ İKİ SATIR İLE ÇİZİLEN GRAFİĞİ WEB'DE GÖSTERİYORUZ.
...
def fetch_btc_data(days=90):
    try:
        ...
        df["timestamp"] = pd.to_datetime(df["timestamp"], unit="ms")
        df = df.set_index("timestamp")
        # Save to CSV automatically for fallback
        df.to_csv("btc_data.csv")
        return df

    except Exception as e:
        print(f"⚠️ API'dan veri çekilemedi: {e}")
        print("📂 CSV dosyasından veri okunuyor...")
        return pd.read_csv("btc_data.csv", index_col="timestamp", parse_dates=True)
